...
int main(int argc, char* argv[])
{
	...
	if (!scene->LoadOBJ(GeometryType::TRIANGLEMESH, position, rotation, scale, filePathTriangleMesh)) {
		std::cout << "ERROR: Could not load Triangle Mesh Geometry. Please check the geometry type or if the file exists at the path provided." << std::endl;
	}
	// LOAD CUBE
	...
	if (!scene->LoadOBJ(GeometryType::CUBE, cubePosition, cubeRotation, cubeScale)) {
		std::cout << "ERROR: Could not load Cube Geometry. Please check the geometry type or if the file exists at the path provided." << std::endl;
	}
	// LOAD PLANE
	...
	if (!scene->LoadOBJ(GeometryType::PLANE, planePosition, planeRotation, planeScale)) {
		std::cout << "ERROR: Could not load Plane Geometry. Please check the geometry type or if the file exists at the path provided." << std::endl;
	}
	// LOAD SPHERE
	...
	if (!scene->LoadOBJ(GeometryType::SPHERE, spherePosition, sphereRotation, sphereScale)) {
		std::cout << "ERROR: Could not load Sphere Geometry. Please check the geometry type or if the file exists at the path provided." << std::endl;
	}
	// END TEST
