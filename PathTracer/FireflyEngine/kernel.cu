...
__device__ void generateRays(Camera* camera, Geometry* geometries, glm::vec3* pixels)
{
	...
	float scale = tan(glm::radians(camera->m_fov * 0.5));
	float imageAspectRatio = camera->m_screenWidth / (float)camera->m_screenHeight;
		...
		case GeometryType::TRIANGLEMESH:
			for (int i = 0; i < geometry.m_numberOfTriangles; ++i)
			{
				if (intersectTriangle(geometry.m_triangles[i], objectSpaceRay, intersect))
				{
					...
					{
						intersect.m_normal = geometry.m_modelMatrix * glm::vec4(intersect.m_normal, 0.f);
						pixels[pixelIndex] = (glm::normalize(glm::abs(intersect.m_normal)));
						tMax = intersect.m_t;
						continue;
					}
				}
...
{
	generateRays(state->d_camera, state->d_geometry, state->d_pixels);
}
